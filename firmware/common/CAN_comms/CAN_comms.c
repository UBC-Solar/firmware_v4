...
CAN_comms_diagnostics_t CAN_comms_diagnostic = {.comms_init_error = COMMS_INIT_SUCCESS,
                                                .dropped_rx_msg = 0,
                                                .rx_queue_count = 0,
                                                .success_rx = 0,
                                                .success_tx = 0,
 ...
 * This function copies the config struct to the global variable and creates the necessary queues,
 * semaphores, and tasks.
 *
 * @attention This function needs to be called by the user AFTER osKernelInitialize() and BEFORE
 * osKernelStart().
 *
...
void CAN_comms_init(CAN_comms_config_t* config)
{
    ...
    /* Create Semaphores */
    CAN_comms_Tx_mailbox_semaphore =
        osSemaphoreNew(NUM_CAN_TX_MAILBOXES, NUM_CAN_TX_MAILBOXES, NULL);
...
void CAN_comms_Add_Tx_message(CAN_comms_Tx_msg_t* CAN_comms_Tx_msg)
{
    ...
    if (HAL_OK ==
        HAL_CAN_AddTxMessage(
            CAN_comms_config.hcan, &CAN_comms_Tx_msg->header, CAN_comms_Tx_msg->data, &canMailbox))
    {
...
void CAN_comms_Rx_message_pending_ISR()
{
    ...
    if (HAL_OK != HAL_CAN_GetRxMessage(
                      CAN_comms_config.hcan, CAN_RX_FIFO0, &CAN_Rx_msg.header, CAN_Rx_msg.data))
    {
 ...
 * This function copies data from the global CAN_comms_diagnostic struct into a
 * CAN_comms_diagnostic_t struct created by the user.
 *
