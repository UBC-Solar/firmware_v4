...
static bool check_efuse_faults(FaultSource_t faults)
{
    if (!(faults & (FAULT_DRD_FUSE | FAULT_MDI_FUSE | FAULT_SPARE_FUSE | FAULT_SPARE_CTRL_FUSE | FAULT_SPARE_MUX_FUSE)))
    {
        return false;
    }
    ...
    if (faults & FAULT_SPARE_MUX_FUSE)  DEBUG_IO_PRINT("FAULT: SPARE_MUX_FUSE tripped\r\n");
    return true;
}
