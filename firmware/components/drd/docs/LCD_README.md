...
There is a indicator on the drive page and all faults in the fault page.

Call the `LCD_display_fault_indicator(g_lcd_batt_faults, g_lcd_motor_faults)` function on the drive page initialized in FreeRTOS to display an indicator

And Call the `LCD_display_faults(g_lcd_batt_faults, g_lcd_motor_faults)` function on the fault page to display faults.

* `g_lcd_batt_faults`: The battery faults included in a struct of each fault with a boolean of whether the fault occurs or not.
* `g_lcd_motor_faults`: The motor faults included in a struct of each fault with a boolean of whether the fault occurs or not.

### Displaying the Warnings
There is a indicator on the drive page and all warnings in the warnings page.

Call the`LCD_display_warning_indicator(g_lcd_warnings)` function on the drive page initialized in FreeRTOS to display an indicator.

And Call the `LCD_display_warnings(g_lcd_warnings)` function on the warnings page to display faults.

* `g_lcd_warnings`: All warnings included in a struct of each warning with a boolean of whether the warning occurs or not.

### Displaying the Temperature
Call the`LCD_display_temperature(temperature_data)` function for each temperature you want to display.

* `temperature_data`: A struct containing the temperature and an id uint8_t variable to identify what temperature it is. Each id is callibrated to a certain location on the LCD Screen. 
