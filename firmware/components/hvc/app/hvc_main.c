    ...
    CAN_HandleTypeDef *_hcan) 
{
    ...
    if (HAL_GPIO_ReadPin(ESTOP_GPIO_Port, ESTOP_Pin) == GPIO_PIN_SET) {
        DEBUG_IO_PRINT("FAULTING from Init because: ESTOP_Pin: %d \r\n", HAL_GPIO_ReadPin(ESTOP_GPIO_Port, ESTOP_Pin));
        ESTOPCallback();
    }
    if (HAL_GPIO_ReadPin(IMD_GPIO_IN_GPIO_Port, IMD_GPIO_IN_Pin) == GPIO_PIN_SET) {
        DEBUG_IO_PRINT("FAULTING from Init because: IMD_GPIO_IN_Pin: %d \r\n", HAL_GPIO_ReadPin(IMD_GPIO_IN_GPIO_Port, IMD_GPIO_IN_Pin));
        IMDFaultCallback();
    }
    if (HAL_GPIO_ReadPin(HV_CURRENT_ALERT_GPIO_Port, HV_CURRENT_ALERT_Pin) == GPIO_PIN_SET) {
        DEBUG_IO_PRINT("FAULTING from Init because: HV_CURRENT_ALERT_Pin: %d \r\n", HAL_GPIO_ReadPin(HV_CURRENT_ALERT_GPIO_Port, HV_CURRENT_ALERT_Pin));
        HVCurrentAlertCallback();
    } // conditionally overrides the value set by HVC_FSM_Init
