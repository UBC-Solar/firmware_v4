...
void CheckForEmergency(module_t *pack_modules, faults_t *pack_faults, warnings_t *pack_warnings) {
    ...
    for (int i = 0; i < NUM_MODULES; i++) {
        ...
        module->faults.bits.fault_over_temperature = module->temperature_mC >= MAX_DISCHARGE_TEMP_degC;
        module->faults.bits.fault_under_temperature = module->temperature_mC <= MIN_TEMPERATURE_degC;
        ...
        module->warnings.bits.warn_high_voltage = module->temperature_mC >= WARN_DISCHARGE_TEMP_degC;
        module->warnings.bits.warn_discharge_high_temperature = module->temperature_mC >= WARN_CHARGE_TEMP_degC;
        module->warnings.bits.warn_charge_high_temperature = module->temperature_mC >= MAX_CHARGE_TEMP_degC;
